...
def get_examples():
    return [[os.path.join(os.path.dirname(__file__), "projects", x[0])] for x in example_projects]
...
def generate(image_array, prompt):
    ...
    messages = [
        {"role": "system", "content": "You are an assistant who perfectly describes images."},
        {
            "role": "user",
            "content": [
                {"type": "image_url", "image_url": {"url": f"file://{TEMP_FILEPATH}"}},
...
def generate_description(image_array):
    return generate(image_array, "Briefly describe the non-text elements in this image.")
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
      ...
      with gr.Column(scale=4):
          ...
          describe_button.click(fn=generate_description, inputs=[image_selector], outputs=chatbot)
          read_text_button = gr.Button("Read Text in Image")
          read_text_button.click(fn=generate_text, inputs=[image_selector], outputs=chatbot)
          prompt = gr.Textbox(lines=2, label="Prompt")
          custom_prommpt_button = gr.Button("Run Custom Prompt")
          custom_prommpt_button.click(fn=generate, inputs=[image_selector, prompt], outputs=chatbot)
